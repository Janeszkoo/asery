...
export function Contact() {
  ...
  useEffect(() => {
    ...
    const trigger = ScrollTrigger.create({
      ...
      onEnter: () => {
        ...
        // Title fade in (not character by character to avoid layout issues)
        tl.fromTo(
          titleRef.current,
          { opacity: 0, y: 20 },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(
        'Failed to send message. Please check the EMAIL.md file for setup instructions.'
      );
    } finally {
    ...
    >
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-8 relative">
          ...
          >
            {/* Title - Fixed size to prevent layout issues */}
            <h2
              ref={titleRef}
              className="text-h2 text-white font-medium mb-4"
            >
              {contactConfig.title}
            </h2>
            ...
            <div className="space-y-8">
              ...
              >
                ...
                <input
                  ...
                  onBlur={() => setFocusedField(null)}
                  className="w-full bg-transparent border-b border-white/20 py-3 text-white placeholder:text-white/30 focus:outline-none focus:border-white transition-colors duration-300"
                />
                <div
                  className={`absolute bottom-0 left-0 h-px bg-white transition-all duration-300 ${
            ...
            <button
              ...
              className="mt-12 px-10 py-4 bg-white text-black text-body font-medium flex items-center gap-3 hover:bg-highlight hover:text-white transition-colors duration-300 relative overflow-hidden group disabled:opacity-70 disabled:cursor-not-allowed"
            >
              <span className="relative z-10">
                {isSubmitting ? 'Sending...' : contactConfig.submitButtonText}
              </span>
              <Send className={`w-5 h-5 relative z-10 ${!isSubmitting ? 'group-hover:translate-x-1 transition-transform duration-300' : ''}`} />
